...
import BatterChargerModel from "../models/add_batteri_charger_model";
...
const addChargingStation = async (req: Request, res: Response) => {
  try {
    ...
    const newChargingStation = new BatterChargerModel({
      userId,
      location,
      latitude,
      longitude,
...
const getChargerById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const chargingStation = await BatterChargerModel.findById(chargerId);
    if (!chargingStation) {
...
const getChargersByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const chargers = await BatterChargerModel.find({ userId });
...
const getAllChargers = async (req: Request, res: Response) => {
  try {
    const chargers = await BatterChargerModel.find();
    if (!chargers || chargers.length == 0) {
...
const deleteChargerById = async (req: Request, res: Response) => {
  try {
    ...
    const charger = await BatterChargerModel.findById(chargerId);
    if (!charger) {
      return res.status(404).json({ message: "Charging station not found" });
    }
    ...
    await BatterChargerModel.findByIdAndDelete(chargerId);
